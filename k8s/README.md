...
| `job-10-extract-vlm-guidance-full.yaml` | **superseded by job-12**, see below | in_clip-bisection + free-generation approach (`extract_vlm_guidance_full.py`); stopped 2026-09-03 at 580/11979 in favor of job-12's real-pairing + teacher-forced design -- kept for reference, not run anymore |
| `job-12-extract-vlm-guidance-paired.yaml` | Job `ruixin-extract-vlm-guidance-paired`, `/data/vans_work/vlm_guidance_cache_paired/*.npz` | full-scale VLM guidance extraction using VANS's real in_clip->out_clip pairing and real correct_caption text (`vans_qa/full_scale/extract_vlm_guidance_paired.py`, a new file, not a modification of job-10's pipeline) -- has its own native per-clip decode timeout, no ThinkJEPA patch needed; see the script's module docstring for the full rationale |
| `job-11-train-qa-jepa-film-gatelr.yaml` | Job `ruixin-train-qa-jepa-film-gatelr`, `raw_data/qa_full_runs_temporal_film_gatelr/jepa/{best.pt,step_<N>.pt}` | jepa x film x middle4 with `--gate_lr_multiplier 10` -- tests whether the gate's slow opening (observed in job-07's completed run: modulator weights moved substantially but gates stayed within +-0.003 of zero-init) is the val_acc bottleneck. **Completed 2026-09-05**: final gates only reached ~0.0099 magnitude (vs baseline's ~0.0025), val_acc 0.61 vs baseline's 0.48-0.60 range -- within noise. Conclusion: FiLM's gradient bandwidth is architecturally limited, not merely slow to open; not a training-dynamics fix |
| `job-13-train-qa-jepa-crossattn.yaml` | Job `ruixin-train-qa-jepa-crossattn`, `raw_data/qa_full_runs_temporal_crossattn/jepa/{best.pt,step_<N>.pt}` | jepa x cross_attn x middle4 on the temporal-hard split -- the clean apples-to-apples counterpart to job-07-train-qa-jepa-film.yaml (same split/layer_strategy/epochs/val schedule). The only prior temporal-split cross_attn checkpoint (`qa_full_runs_temporal_crossattn_test/`) was an interrupted 313-step run, not a fair comparison against job-07/job-11's ~6000+ step budget -- deleted 2026-09-06 and replaced by this full run, submitted following the job-11 gate_lr conclusion above |
| `job-14-smoke-latent-world-model.yaml` | Job `ruixin-smoke-latent-world-model`, `/data/raw_data/latent_world_model_smoke/{crossattn,none}/` | smoke test for `vans_qa/full_scale/train_latent_world_model_full.py` (new file) -- the VLM-guides-JEPA direction's first-ever full-scale training script: does real VLM guidance (from job-12's output) actually improve V-JEPA2's self-supervised future-latent MSE vs. a no-guidance control, using `CortexGuidedVideoPredictor` unmodified. The prior demo script (`vans_qa/demo/train_latent_world_model.py`) read the wrong cache schema/shapes (hardcoded (64,128) tokens vs. the real (16,256)), had no no-guidance control, and hardcoded film mode -- see the new script's module docstring. **Completed 2026-09-07** after fixing a missing `h5py` import (`cache_train.thinker_train` unconditionally imports EgoDex's dataloader) -- both conditions ran cleanly end to end, confirming shapes/imports are correct |
| `job-15-train-latent-world-model-crossattn.yaml` / `job-16-train-latent-world-model-none.yaml` | Jobs `ruixin-train-latent-world-model-{crossattn,none}`, `raw_data/latent_world_model_runs/{crossattn,none}/{best.pt,step_<N>.pt,test_metrics.json}` | full-scale VLM-guides-JEPA training on job-12's real output (11777/12032 pairs -- job-12 was stopped there 2026-09-06, the remaining ~255 failures confirmed deterministic/permanent decode errors on the same pids across two independent attempts, not worth continuing to retry). job-15 uses `--guidance crossattn` (consistent with the forward direction's job-11 conclusion that FiLM is architecturally bandwidth-limited there); job-16 is the matched `--guidance none` control, on a separate conda PVC (`ruixin-conda-vol`, not `-vlm`) so both can run concurrently. **Both completed 2026-09-08**: crossattn beat the no-guidance control on all three held-out test metrics (pred_loss 7.4503 vs 7.4691, latent_dist 87.044 vs 87.150, cosine_dist 0.4902 vs 0.4920) -- a small but consistent, monotonically-widening gap from step 1000 onward, not noise. First quantitative evidence in this project that VLM guidance actually helps JEPA's future-latent prediction |
| `job-17-fit-warmstart-alignment.yaml` | Job `ruixin-fit-warmstart-alignment`, `raw_data/warmstart_alignment.pt` | fits `vans_qa/full_scale/fit_warmstart_alignment.py`'s closed-form ridge-regression alignment between pooled V-JEPA2 features and pooled VLM hidden states, reusing job-12's already-extracted pairs (no GPU, no new extraction). Feeds job-18/job-19's `--warmstart_alignment` flag -- must run and complete first |
| `job-18-train-qa-jepa-crossattn-warmstart.yaml` / `job-19-train-latent-world-model-crossattn-warmstart.yaml` | Jobs `ruixin-train-{qa-jepa,latent-world-model}-crossattn-warmstart`, `raw_data/qa_full_runs_temporal_crossattn_warmstart/jepa/` and `raw_data/latent_world_model_runs_warmstart/crossattn/` | the warm-start cross-modal coupling ablation (idea 2 from the earlier research planning): identical configs to job-13 and job-15 respectively, but with `--warmstart_alignment` set, initializing each direction's cross-modal adapter from job-17's fitted alignment instead of random init. job-13/job-15's already-completed runs are this ablation's random-init baselines -- compare directly against those. **2026-09-09**: job-19 (reverse) reached step 8000/8253, statistically indistinguishable from job-15's trajectory (trending slightly worse) -- no measurable benefit. job-18 (forward) reached step 5000/6688; job-13's own run turned out to have crashed mid-validation at step 5000 (never actually completed, discovered only now -- see job-20), so the comparison is only valid through step 4500, where both trajectories bounce noisily around chance level with several suspicious exact ties |
| `job-20-check-alignment-holdout.yaml` | Job `ruixin-check-alignment-holdout` (diagnostic, writes nothing to disk) | held-out R^2 check for job-17's alignment fit (`fit_warmstart_alignment.py --holdout_frac`), run because job-18/job-19 showed no benefit from warm-start and the in-sample R^2 job-17 reported (0.84-0.998) can be badly optimistic -- rev_old/rev_new regress a 2048-dim input against ~1463 samples, more free parameters than data points. **Result**: held-out R^2 was -0.25 and -1.21 for the reverse direction (pure overfitting noise, worse than predicting the mean) and 0.33 for the forward direction (a real but modest signal, judged too weak to build on further with this method). job-18/job-19 stopped early per the user's call; warm-start initialization abandoned as the coupling mechanism in favor of job-21/job-22's joint training |
| `job-21-smoke-joint-coupled.yaml` | Job `ruixin-smoke-joint-coupled`, `/data/raw_data/joint_coupled_smoke/` | smoke test for `vans_qa/full_scale/train_joint_coupled.py` (new file) -- replaces warm-start initialization with joint/multi-task training: a shared `CrossModalBridge` (new file, `common/cross_modal_bridge.py`) trained live by both directions at once (each direction's own task loss plus a cycle-consistency term), instead of a one-shot closed-form fit before training. Reuses `train_qa_full.py` and `train_latent_world_model_full.py` entirely via import -- neither is modified, both remain the independent-training baselines this gets compared against. Flags the real GPU-memory risk of loading Qwen3-VL-2B (~10.5GB) and `CortexGuidedVideoPredictor` in the same process -- run this smoke test before committing to a full run |
| `job-22-train-joint-coupled.yaml` | Job `ruixin-train-joint-coupled`, `/data/raw_data/joint_coupled_runs/{train_log.jsonl,best_forward.pt,best_reverse.pt,step_<N>.pt,rev_test_metrics.json}` | full-scale joint/multi-task training run, validated by job-21's smoke test. Compared against a clean rerun of job-13 (forward baseline) and job-15 (reverse baseline, already complete). Uses `ruixin-conda-vol-vlm` (not `ruixin-conda-vol`, held by the concurrently-running job-13) -- the first submission used the wrong PVC and sat stuck 2+ hours on a `FailedAttachVolume` error that was misread at the time as a bad-node problem (see the two `hcc-nrp-shor-*` exclusions added and left in across every job template -- harmless but not the real cause). **In progress 2026-09-10**: early checkpoints (steps 500-2500) show the reverse direction's val_mse consistently at or below job-15's baseline at every matching step (small but consistent edge); forward val_acc is noisier and mixed against job-13 |
| `job-23-smoke-coin-longaxis.yaml` | Job `ruixin-smoke-coin-longaxis`, `/data/raw_data/coin_longaxis_smoke/` | smoke test for `vans_qa/full_scale/extract_coin_longaxis.py` (new file) -- a genuinely long-horizon data source for the reverse direction's self-supervised leg, built on COIN's own public step annotations (`COIN.json`, no license needed for the annotations -- only Tsinghua's pre-packaged video mirror requires one) rather than VANS's own short-step pairs (median ~3-4 clip-index span) or EgoDex (no step-level QA/captions at all). COIN's own first-to-last-step span is a real median 71s (p90 154s, max 733s), and a 60-video YouTube-availability sample came back ~82% still live. No VLM/Qwen3-VL step at all (pure V-JEPA2 self-supervised features, no captions) -- output has no `vlm_old`/`vlm_new` fields and needs `--guidance none` wherever it's consumed. First run hit two real bugs (fixed): `uniform_indices(start, end, n)`'s second argument is an absolute end frame index, not a length; and yt-dlp needs a "supported" JS runtime to see YouTube's real format list at all -- neither of the image's node builds (existing v16, nor a freshly downloaded v20) satisfy yt-dlp's own JS Challenge Provider check, only `deno` (downloaded fresh into `/tmp` each run) works, combined with an explicit `bestvideo+bestaudio` merge (`--merge-output-format mp4`) since modern YouTube rarely serves a single muxed stream anymore. **Result 2026-09-11**: 5/5 candidates saved successfully end to end (real download -> two-window decode -> V-JEPA2 encode -> atomic `.npz` write) -- extraction script validated, ready for a full-scale run |
