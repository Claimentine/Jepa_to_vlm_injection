...
| `job-10-extract-vlm-guidance-full.yaml` | Job `ruixin-extract-vlm-guidance-full`, `/data/vans_work/vlm_guidance_cache_full/*.npz` | full-scale VLM guidance extraction (`vans_qa/full_scale/extract_vlm_guidance_full.py`); currently `--limit 50` for a timing/GPU-memory check, remove the cap for the full ~12k-clip run once that looks healthy |
