...
| `job-20-check-alignment-holdout.yaml` | Job `ruixin-check-alignment-holdout` (diagnostic, writes nothing to disk) | held-out R^2 check for job-17's alignment fit (`fit_warmstart_alignment.py --holdout_frac`), run because job-18/job-19 showed no benefit from warm-start and the in-sample R^2 job-17 reported (0.84-0.998) can be badly optimistic -- rev_old/rev_new regress a 2048-dim input against ~1463 samples, more free parameters than data points. Answers "is there real cross-modal signal to exploit at all" before designing a different warm-start variant. Runs a standalone CPU-only torch install instead of mounting either conda PVC, both held by job-18/job-19 |
