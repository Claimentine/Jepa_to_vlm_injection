...
| `job-11-train-qa-jepa-film-gatelr.yaml` | Job `ruixin-train-qa-jepa-film-gatelr`, `raw_data/qa_full_runs_temporal_film_gatelr/jepa/{best.pt,step_<N>.pt}` | jepa x film x middle4 with `--gate_lr_multiplier 10` -- tests whether the gate's slow opening (observed in job-07's completed run: modulator weights moved substantially but gates stayed within +-0.003 of zero-init) is the val_acc bottleneck |
